...
function juiz_odpm_activation() {
	...
	if ( !is_array($juiz_sma_options) ) {
		...
		$default_array = array(
			...
			'hide_metabox'	 		=> 'n',
			'outdated_message' 		=> __( 'This content has ##. Please, read this page keeping its age in your mind.', 'juiz-outdated-post-message' )
		);
...
if ( ! function_exists( 'add_juiz_odpm_settings_page' ) ) {
	...
	function add_juiz_odpm_settings_page() {
		add_submenu_page( 
			'options-general.php', 
			__( 'Outdated Post Message', 'juiz-outdated-post-message' ),
			__( 'Outdated Post Message', 'juiz-outdated-post-message' ),
			'administrator',
			JUIZ_ODPM_SLUG,
		...
		if ( is_array( $options['post_type_concerned'] ) ) {
			foreach ( $all_pts as $pt ) {
				if ( in_array( $pt, $options['post_type_concerned'] ) ) {
					add_meta_box( 'jodpm_options', __( 'Outdated message Options', 'juiz-outdated-post-message' ), 'jodpm_options', $pt, 'side', 'default' );
				}
			}
		}
	}
...
if ( ! function_exists( 'jodpm_options' ) ) {
	function jodpm_options( $post ) {
		...
		echo '<p><input id="jodpm_metabox_hide_message" type="checkbox"' . $checked . ' name="jodpm_metabox_hide_message" /> <label for="jodpm_metabox_hide_message">' . __( 'Never show the outdated message for this post', 'juiz-outdated-post-message' ) . '</label></p>';

		echo '<p><label for="jodpm_metabox_custom_delay" style="font-weight:bold;">' . __( 'This post delay:', 'juiz-outdated-post-message' ) . '</label> <input id="jodpm_metabox_custom_delay" style="width:65px" type="number" name="jodpm_metabox_custom_delay" step="5" min="0" value="' . $custom_d . '" />' . __( 'days', 'juiz-outdated-post-message' ) . '</p>
			<p class="howto">' . sprintf( __( 'Keep 0 to use global value (%s)', 'juiz-outdated-post-message' ), $options['delay_before_outdated'] . '&nbsp;' . __( 'days', 'juiz-outdated-post-message' ) ) . '</p>';
	}
}
// save datas
...
function add_juiz_odpm_plugin_options() {
	...
	add_settings_section('juiz_odpm_plugin_basics', __( 'General Settings', 'juiz-outdated-post-message' ), 'juiz_odpm_section_text_basics', JUIZ_ODPM_SLUG );
	add_settings_field('juiz_odpm_delay', '<label for="juiz_odpm_delay">'.__('Delay before outdated:', 'juiz-outdated-post-message').'</label>', 'juiz_odpm_setting_delay', JUIZ_ODPM_SLUG, 'juiz_odpm_plugin_basics');
	add_settings_field('juiz_odpm_message', '<label for="juiz_odpm_message">'.__('Your message:', 'juiz-outdated-post-message').'</label>', 'juiz_odpm_setting_message', JUIZ_ODPM_SLUG, 'juiz_odpm_plugin_basics');
	add_settings_field('juiz_odpm_temp_submit_1', get_submit_button(__('Save Changes'), 'secondary'), function() {return "";}, JUIZ_ODPM_SLUG, 'juiz_odpm_plugin_basics');


	add_settings_section('juiz_odpm_plugin_display_in', __('Display settings','juiz-outdated-post-message'), 'juiz_odpm_section_text_display', JUIZ_ODPM_SLUG);
	add_settings_field('juiz_odpm_display_in_types', __('Display message in:','juiz-outdated-post-message'), 'juiz_odpm_setting_checkbox_content_type', JUIZ_ODPM_SLUG, 'juiz_odpm_plugin_display_in');
	add_settings_field('juiz_odpm_display_where', __('Display message to the:','juiz-outdated-post-message'), 'juiz_odpm_setting_radio_where', JUIZ_ODPM_SLUG, 'juiz_odpm_plugin_display_in');
	add_settings_field('juiz_odpm_temp_submit_2', get_submit_button(__('Save Changes'), 'secondary'), function() {return "";}, JUIZ_ODPM_SLUG, 'juiz_odpm_plugin_display_in');


	add_settings_section('juiz_odpm_plugin_metaboxes', __('Metabox Settings','juiz-outdated-post-message'), 'juiz_odpm_section_text_metaboxes', JUIZ_ODPM_SLUG);
	add_settings_field('juiz_odpm_hide_metabox', __('Hide the metabox?','juiz-outdated-post-message'), 'juiz_odpm_setting_hide_metabox', JUIZ_ODPM_SLUG, 'juiz_odpm_plugin_metaboxes');
...
function juiz_odpm_section_text_basics() {
	echo '<p class="juiz_odpm_section_intro">' . __( 'Set general options for your outdated message.', 'juiz-outdated-post-message' ) .'</p>';
}
}
...
function juiz_odpm_setting_delay() {
	...
	echo '<p><input type="number" name="' . JUIZ_ODPM_SETTING_NAME . '[delay_before_outdated]" id="juiz_odpm_delay" value="' . $options['delay_before_outdated'] . '" class="juiz_short_input" step="5" min="15">' . __( 'days', 'juiz-outdated-post-message' ) . '</p>';
}
}
...
function juiz_odpm_setting_message() {
	$options = get_option( JUIZ_ODPM_SETTING_NAME );
	echo '<p><textarea class="juiz_long_input" name="' . JUIZ_ODPM_SETTING_NAME . '[outdated_message]" id="juiz_odpm_message">' . $options['outdated_message'] . '</textarea><br><em>' . sprintf( __( 'You can use the %s##%s variable to insert the time passed since the publication date.', 'juiz-outdated-post-message' ), '<strong>', '</strong>' ) . '</em></p>';
}
}
...
function juiz_odpm_section_text_display() {
	echo '<p class="juiz_odpm_section_intro">' . __( 'Choose where you want to display the outdated message on your posts', 'juiz-outdated-post-message' ) . '</p>';
}
}
...
if ( ! function_exists( 'juiz_odpm_setting_radio_where' ) ) {
	function juiz_odpm_setting_radio_where() {
		...
		echo '	<input id="jodpm_w_b" value="bottom" name="' . JUIZ_ODPM_SETTING_NAME . '[where_to_display]" type="radio" ' . $w_bottom . ' />
				<label for="jodpm_w_b">' . __( 'Content bottom', 'juiz-outdated-post-message' ) . '</label>
				
				<input id="jodpm_w_t" value="top" name="'.JUIZ_ODPM_SETTING_NAME.'[where_to_display]" type="radio" '.$w_top.' />
				<label for="jodpm_w_t">'. __( 'Content top', 'juiz-outdated-post-message' ) . '</label>
				
				<input id="jodpm_w_2" value="both" name="' . JUIZ_ODPM_SETTING_NAME . '[where_to_display]" type="radio" ' . $w_both . ' />
				<label for="jodpm_w_2">' . __( 'Both', 'juiz-outdated-post-message' ) . '</label>

				<input id="jodpm_w_0" value="nowhere" name="'.JUIZ_ODPM_SETTING_NAME.'[where_to_display]" type="radio" '.$w_nowhere.' />
				<label for="jodpm_w_0">' . __( 'I\'m a ninja, I want to use the shortcode only!', 'juiz-outdated-post-message' ) . '</label>';
	}
}
...
function juiz_odpm_section_text_metaboxes() {
	echo '<p class="juiz_odpm_section_intro">' . __('You can hide metabox added by this plugin in your posts\' edition pages', 'juiz-outdated-post-message' ) . '</p>';
}
}
// radio fields styles choice
if ( ! function_exists( 'juiz_odpm_setting_hide_metabox' ) ) {
	function juiz_odpm_setting_hide_metabox() {
		...
		echo '	<input id="jodpm_hide_y" value="y" name="' . JUIZ_ODPM_SETTING_NAME . '[hide_metabox]" type="radio" ' . $this_y . ' />
				<label for="jodpm_hide_y">' . __( 'Yes', 'juiz-outdated-post-message' ) . '</label>
				
				<input id="jodpm_hide_n" value="n" name="' . JUIZ_ODPM_SETTING_NAME . '[hide_metabox]" type="radio" ' . $this_n . ' />
				<label for="jodpm_hide_n">' . __( 'No', 'juiz-outdated-post-message' ) . '</label>';
	}
}
	...
	function juiz_odpm_settings_page() {
		...
		<div id="juiz-odpm" class="wrap">
			...
			<div class="error settings-error">
				<p>
					<strong><?php echo sprintf( __( 'You must chose at least one %stype of content%s.', 'juiz-outdated-post-message' ), '<a href="#post">', '</a>' ); ?></strong><br>
					<em><?php esc_html_e( 'If you want to deactivate the outdated message, be a ninja!','juiz-outdated-post-message' ); ?></em></p>
			</div>
			<?php } ?>
			<p class="jodpm_info">
				<?php echo sprintf( __( 'You can use %s[outdated]%s shortcode to display the outdated message.','juiz-outdated-post-message' ), '<code>','</code>'); ?>
